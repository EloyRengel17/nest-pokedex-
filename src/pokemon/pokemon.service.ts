...
export class PokemonService {
  ...
  async findOne(id: string) {
    ...
    if (!pokemon) throw new NotFoundException(`POkemon with name or id not found `)
  ...
  async update(term: string, updatePokemonDto: UpdatePokemonDto) {
    ...
    try {
      const updatedPokemon = await this.pokemonModel.findByIdAndUpdate(pokemon._id, updatePokemonDto, { new: true });
  ...
  private handleException(error: any) {
    if (error.code === 11000) {
      throw new BadGatewayException(`there is one pokemon whit this no:  ${JSON.stringify(error.keyValue)}`)
